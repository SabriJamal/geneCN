...
python $cnv_kit_path/cnvkit.py batch $sample_blood -r $pooled_ref \
...
sample_name=$(echo $sample_blood | cut -d"/" -f6 | cut -d"." -f1)
...
#####################################
cat << EOF > $mydirec/Result/CNV_kit_$experiment\_$id/cnvkit_call_bloods.out.sh 
