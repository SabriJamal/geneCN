...
def get_CNA(CNA_in, CNA_out, bed, thresh=0.2):
	file_in = CNA_in
	file_out = CNA_out
	CNA_dict = {}
	header = 0
	...
	with open(file_out, "w") as OUT:
		for gene_name,val_array in CNA_dict.items():
			...
			for i in range(0, len(val_array[0])):
				...
				for k in range(0, len(val_array[1][i])):
				...
				OUT.write(gene_name + "\t" + str(min_log2) + "\t" + str(max_log2) + "\t" + str(nr_ROI) + "\t" + str(del_exon_counter) + "\t" + del_exon_names + "\t" + str(amp_exon_counter) + "\t" + amp_exon_names + "\t" + str(del_exon_perc) + "\t" + str(amp_exon_perc) + "\n")

		#for item in val[0]:
		#	try:
		#		if(int(item)):

		#			print("type conversion worked for: " + str(item))
		#	except:
		#		print("type conversion failed for: " + item)
...
#Extract_ROI_per_gene("/Users/sjamal/Documents/Work/2.scripts/geneCN/CNVkit_tests/Result/PooledReference/CNV_kit_ABCBIO_9600/ABCBIO.reference.target-tmp.bed")
